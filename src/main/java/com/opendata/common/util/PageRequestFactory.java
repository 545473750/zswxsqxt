...
public class PageRequestFactory {
    ...
    public static PageRequest bindPageRequest(PageRequest pageRequest, HttpServletRequest request,String defaultSortColumns, int defaultPageSize) {
	    	try {
	    		...
	    		Iterator keyIterator = params.keySet().iterator();//去掉查询时的空格
	    		while(keyIterator.hasNext()){
	    			Object temp = keyIterator.next();
	    			if(temp!=null&&params.get(temp)!=null){
	    				params.put(temp, params.get(temp).toString().trim());
	    			}
