...
@Namespace("/organiz")
@Results({
@Result(name="moveDialogclose", location="/WEB-INF/pages/organiz/organization/moveDialogClose.jsp", type="dispatcher"),
@Result(name="list",location="/WEB-INF/pages/organiz/organization/list.jsp", type = "dispatcher"),

@Result(name="query",location="/WEB-INF/pages/organiz/organization/query.jsp", type = "dispatcher"),
@Result(name="create",location="/WEB-INF/pages/organiz/organization/create.jsp", type = "dispatcher"),
@Result(name="edit",location="/WEB-INF/pages/organiz/organization/edit.jsp", type = "dispatcher"),
@Result(name="show",location="/WEB-INF/pages/organiz/organization/show.jsp", type = "dispatcher"),
@Result(name="move", location="/WEB-INF/pages/organiz/organization/move.jsp", type="dispatcher"),

@Result(name="main",location="/WEB-INF/pages/organiz/organization/main.jsp", type = "dispatcher"),
@Result(name="list_partition",location="/WEB-INF/pages/organiz/organization/list_partition.jsp", type = "dispatcher"),
@Result(name="leftTree",location="/WEB-INF/pages/organiz/organization/leftTree.jsp", type = "dispatcher"),
@Result(name="rightpage",location="/WEB-INF/pages/organiz/organization/rightPage.jsp", type = "dispatcher"),
@Result(name="loaduser",location="/WEB-INF/pages/organiz/organization/rightPage_user.jsp", type = "dispatcher"),
@Result(name="loadjob",location="/WEB-INF/pages/organiz/organization/rightPage_job.jsp", type = "dispatcher"),
@Result(name="loaddept",location="/WEB-INF/pages/organiz/organization/rightPage.jsp", type = "dispatcher"),
...
@Result(name="loadDefaultStation", location="/WEB-INF/pages/organiz/organization/rightPage_job_list.jsp", type="dispatcher"),
@Result(name="loadUserAction", location="..//organiz/Organization!loadUser.do?parentid=${parentOrganizationId}&partitionId=${partitionId}", type="redirectAction"),
@Result(name="configUserAndOrganization", location="/WEB-INF/pages/organiz/organization/dialogclose.jsp", type="dispatcher"),
@Result(name="loadDefaultUser", location="/WEB-INF/pages/organiz/organization/rightPage_user_list.jsp", type="dispatcher"),
@Result(name="listAction",location="..//organiz/Organization!list.do?${params}&flag=refresh", type = "redirectAction"),
@Result(name="delete_torightpage", location="..//organiz/Organization!toRightPage.do?${params}&flag=refresh", type="redirectAction"),
@Result(name="update_torightpage", location="..//organiz/Organization!toRightPage.do?${params}&flag=refresh", type="redirectAction"),
//@Result(name="torightpageAction", location="..//organiz/Organization!toRightPage.do?id=${parentid}&org_type=${org_type}&flag=refresh", type="redirectAction")
@Result(name="torightpageAction", location="..//organiz/Organization!toRightPage.do?${params}&flag=refresh", type="redirectAction")
})
public class OrganizationAction extends BaseStruts2Action implements Preparable,ModelDriven{
...
//		this.organizationList = this.organizationManager.findAllByDf();
		String seq = this.organizationManager.findSequence(query.getParentId());
		this.organization.setSequence(Long.valueOf(seq));
//		organization.setParent_id(query.getParentId());
		return CREATE_JSP;
	}
	...
	public String moveOperation() {
		this.organization = this.organizationManager.getById(this.id);
		/*this.organization.setParent_id(query.getParentId());
		if(this.parentOrganizationId != null && this.parentOrganizationId.equals("root")) {
			this.organization.setParent_id(null);
		}*/
		this.organizationManager.update(this.organization);
	...
	public String update() {
		...
		this.organizationManager.saveOrUpdate(organization);
	...
	public String toRightPage() {
		/*//如果查询分区下的根组织机构就返回空
		if(partitionId!=null&&!"".equals(partitionId)&& this.id.equals("root")){
			return null;
		}
		getRequest().setAttribute("pid", this.id);
		getRequest().setAttribute("organizationModel", this.organizationManager.getById(id));
		getRequest().setAttribute("flag", this.flag);*/
		query.setDf("0");
		
		// 如果是root,则显示下级列表
		// 如果当前组织机构没有下级组织机构，则显示当前组织机构信息
		// 如果当前组织机构有下级组织机构，则显示下级列表
		/*if(this.id != null && this.id.equals("root")) {
			query.setParentId(null);
			Page page = this.organizationManager.findPage(query);
			
			// 组织机构类别
			List<Organization> result = page.getResult();
			for(Organization organization : result) {
				organization.setTypeString(dicUtil.getDicList("ORG_ORGTYPE").get(organization.getType()));
			}
			this.saveCurrentPage(page,query);
			return RIGHTPAGE;
		} */
		
//		query.setParentId(this.id);
//		query.setPageNumber(1);
	 ...
	 * @return
	 */
	public String listByPartition() {
		Partition partition = partitionManager.getById(partitionId);
		getRequest().setAttribute("listByPartition", partition.getOrganizations());
		return LIST_PARTITION_JSP;
	}
...
//		}
	...
	public String deptXML() {
		try {
			...
			String menuXmlString = Common.getDeptTreeXML(resultOrgList);
...
//			String menuXmlString = Common.getDeptTreeXML(orgs);
			...
			String xmlString = Common.getOrganizationTreeXml(orgs, this.selectedIds, this.id);
			...
			if(this.selectedIds != null && this.selectedIds.equals("root")) {
				sb.append("<item id=\"root\" text=\"单位\" checked=\"1\">");
			} else {
				sb.append("<item id=\"root\" text=\"部门\">");
			}
			sb.append(tmp);
			sb.append("</item></tree>");
	...
	public Organization getParent(){
		...
		if(StringUtils.isNotBlank(parentId)){
			Organization parent = this.organizationManager.getById(parentId);
			return parent;
		}
		return null;
	}
